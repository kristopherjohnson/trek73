#ident "@(#) TREK73 $Header: /home/ncvs/trek73/src/enemycom.c,v 1.3 1999/10/26 20:21:59 dillon Exp $"
/*
 * $Source: /home/ncvs/trek73/src/enemycom.c,v $
 *
 * $Header: /home/ncvs/trek73/src/enemycom.c,v 1.3 1999/10/26 20:21:59 dillon Exp $
 *
 * $Log: enemycom.c,v $
 * Revision 1.3  1999/10/26 20:21:59  dillon
 *
 *     Fix compiler warnings (for situations that cannot occur)
 *
...
Prototype int e_attack(struct ship *sp, struct ship *fed);
Prototype int e_attack(struct ship *sp, struct ship *fed);
Prototype int e_checkarms(struct ship *sp);
Prototype int e_checkprobe(struct ship *sp);
Prototype int e_cloak_off(struct ship *sp, struct ship *fed);
Prototype int e_cloak_on(struct ship *sp, struct ship *fed);
Prototype int e_closetorps(struct ship *sp, struct ship *fed);
Prototype int e_destruct(struct ship *sp, struct ship *fed);
Prototype int e_evade(struct ship *sp, int x, int y, int type);
Prototype int e_jettison(struct ship *sp, struct ship *fed);
Prototype int e_launchprobe(struct ship *sp, struct ship *fed);
Prototype int e_loadtubes(struct ship *sp);
Prototype int e_lockphasers(struct ship *sp, struct ship *fed);
Prototype int e_locktubes(struct ship *sp, struct ship *fed);
Prototype int e_phasers(struct ship *sp, struct ship *fed);
Prototype int e_pursue(struct ship *sp, struct ship *fed, float speed);
Prototype int e_runaway(struct ship *sp, struct ship *fed);
Prototype int e_torpedo(struct ship *sp);
 ...
 */
int
e_cloak_off(struct ship *sp, struct ship *fed)
{
	if (sp->cloaking != C_ON)
		return 0;
	sp->cloaking = C_OFF;
 ...
 */
int
e_cloak_on(struct ship *sp, struct ship *fed)
{
 ...
 */
int
e_closetorps(struct ship *sp, struct ship *fed)
{
	struct list *lp;
	int range;
 ...
 */
int
e_destruct(struct ship *sp, struct ship *fed)
{
	if (sp->delay < 5.0)
		return 0;
	sp->delay = 5.0;
 ...
 */
int
e_lockphasers(struct ship *sp, struct ship *fed)
{
	int i;
	int banks;
 ...
 */
int
e_locktubes(struct ship *sp, struct ship *fed)
{
	int i;
	int tubes;
