#ident "@(#) TREK73 $Header: /home/ncvs/trek73/src/cmds2.c,v 1.4 1999/10/26 20:14:29 dillon Exp $"
/*
 * $Source: /home/ncvs/trek73/src/cmds2.c,v $
 *
 * $Header: /home/ncvs/trek73/src/cmds2.c,v 1.4 1999/10/26 20:14:29 dillon Exp $
 *
 * $Log: cmds2.c,v $
 * Revision 1.4  1999/10/26 20:14:29  dillon
 *
 *     the helm lock printf in the survivors report was missing an indirection
 *
 * Revision 1.3  1999/10/26 20:12:58  dillon
 *
...
Prototype int pos_report(struct ship *sp);
Prototype int pos_display(struct ship *sp);
Prototype int pursue(struct ship *sp);
Prototype int elude(struct ship *sp);
Prototype int helm(struct ship *sp);
Prototype int self_scan(struct ship *sp);
Prototype int scan(struct ship *sp);

int print_damage(struct ship *ep);
int scan_torpedo(struct torpedo *pp);

int
pos_report(struct ship *sp)
{
	struct	ship *sp1;
	struct	torpedo *tp;
	struct	list *lp;
			...
			switch(lp->type) {
				...
				case I_TORPEDO:
					(void) sprintf(who,"%.*s torp %d  ",
					    (int)sizeof(who) - 11, tp->from->name,
					    tp->id);
					break;
#endif
				case I_PROBE:
					(void) sprintf(who, "%.*s probe %d  ",
					    (int)sizeof(who) - 12, tp->from->name,
					    tp->id);
					break;
				case I_ENG:
					(void) sprintf(who,"%.*s engineering  ",
					    (int)sizeof(who) - 15, tp->from->name);
					break;
				default:
...
int
scan_torpedo(struct torpedo *pp)
{
	char kind[20];
	char tgt[20];
