#ident "@(#) TREK73 $Header: /home/ncvs/trek73/src/globals.c,v 1.3 1999/10/26 20:39:35 dillon Exp $"
/*
 * $Source: /home/ncvs/trek73/src/globals.c,v $
 *
 * $Header: /home/ncvs/trek73/src/globals.c,v 1.3 1999/10/26 20:39:35 dillon Exp $
 *
 * $Log: globals.c,v $
...
Prototype char encstr[];
Prototype char version[];
Prototype char *sysname[];
Prototype char *statmsg[];
Prototype struct damage p_damage;
Prototype struct damage a_damage;
Prototype char *feds[];
Prototype struct race_info aliens[MAXFOERACES];

Prototype float init_p_turn[MAXPHASERS][MAXPHASERS];
Prototype float init_t_turn[MAXTUBES][MAXTUBES];

/*
 * for the linked list of items in space
 */
Prototype struct list head;
Prototype struct list *tail;

/*
 * Global definitions
 */
Prototype float	segment;
Prototype float	timeperturn;

Prototype struct	ship *shiplist[];

Prototype char	home[256];
Prototype char	savefile[256];
Prototype char	captain[30];
Prototype char	science[30];
Prototype char	engineer[30];
Prototype char	com[30];
Prototype char	nav[30];
Prototype char	helmsman[30];
Prototype char	title[9];
Prototype char	foerace[30];
Prototype char	foename[30];
Prototype char	foestype[30];
Prototype char	empire[30];
Prototype char	class[3];
Prototype char	foeclass[3];
Prototype int	shipnum;
Prototype int	enemynum;
Prototype int	terse;
Prototype int	silly;
Prototype int	defenseless;
Prototype int	corbomite;
Prototype int	surrender;
Prototype int	surrenderp;
Prototype int	restart;
Prototype char	shutup[HIGHSHUTUP];
Prototype char	slots[HIGHSLOT];
Prototype int	global;
Prototype char	*options;
Prototype char	sex[20];
Prototype char	shipname[30];
Prototype char	racename[20];
Prototype int	reengaged;
Prototype char    com_delay[6];
Prototype int	teletype;
Prototype int	time_delay;
Prototype int	trace;
Prototype char	can_cloak;
Prototype double e_bpv;

Prototype char Input[BUFSIZ];
Prototype char *Inptr;
Prototype char parsed[BUFSIZ];

Prototype struct ship_stat	us;
Prototype struct ship_stat	them;

Prototype struct  cmd cmds[];
Prototype int high_command;
Prototype int cmdarraysize;
Prototype void (*strategies[])(struct ship *);


char encstr[] = "\211g\321_-\251b\324\237;\255\263\214g\"\327\224.,\252|9\265=\357+\343;\311]\341`\251\b\231)\266Y\325\251";

char version[] = "TREK73 Version 4.1 26-Oct-1999";
